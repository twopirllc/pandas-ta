    ...
    if Imports["yfinance"] and ticker is not None:
        ...
        if kind in _all + ["info"] or verbose:
            ...
            if "companyOfficers" in ticker_info and len(
                ...
                print(
                    f"Company Officers: {', '.join(ticker_info['companyOfficers'])}".ljust(40))
            if "sector" in ticker_info and len(
                    ticker_info["sector"]) and "industry" in ticker_info and len(ticker_info["industry"]):
                print(
            ...
            if "navPrice" in ticker_info and ticker_info[
                    "navPrice"] is not None or "yield" in ticker_info and ticker_info["yield"] is not None:
                print(
            ...
            if "sharesOutstanding" in ticker_info and ticker_info[
                    "sharesOutstanding"] is not None and "floatShares" in ticker_info and ticker_info["floatShares"] is not None:
                print(
            ...
            if "sharesShort" in ticker_info and "shortRatio" in ticker_info and ticker_info[
                    "sharesShort"] is not None and ticker_info["shortRatio"] is not None:
                print(
            ...
            if "shortPercentOfFloat" in ticker_info and ticker_info[
                    'shortPercentOfFloat'] is not None and "sharesShortPriorMonth" in ticker_info and ticker_info['sharesShortPriorMonth'] is not None:
                print(
            ...
            if "heldPercentInstitutions" in ticker_info and ticker_info[
                    'heldPercentInstitutions'] is not None or "heldPercentInsiders" in ticker_info and ticker_info['heldPercentInsiders'] is not None:
                print(
            ...
            print()
            if "bookValue" in ticker_info and ticker_info['bookValue'] is not None and "priceToBook" in ticker_info and ticker_info[
                    'priceToBook'] is not None and "pegRatio" in ticker_info and ticker_info['pegRatio'] is not None:
                print(
            ...
            if "forwardPE" in ticker_info and ticker_info['forwardPE'] is not None:
                print(
                    f"Forward PE".ljust(39),
                    f"{ticker_info['forwardPE']}".rjust(40))
            if "forwardEps" in ticker_info and ticker_info[
                    'forwardEps'] is not None and "trailingEps" in ticker_info and ticker_info['trailingEps'] is not None:
                print(
            ...
            if "enterpriseToRevenue" in ticker_info and ticker_info[
                    'enterpriseToRevenue'] is not None or "enterpriseToEbitda" in ticker_info and ticker_info['enterpriseToEbitda'] is not None:
                print(
            ...
            _o, _h, _l, _c, _v = ticker_info['open'], ticker_info['dayHigh'], ticker_info[
                'dayLow'], ticker_info['regularMarketPrice'], ticker_info['regularMarketVolume']
            print(f"Open High Low | Close".ljust(39),
                  f"{_o:.4f}  {_o:.4f}  {_l:.4f} | {_c:.4f}".rjust(40))
            print(
                f"HL2 | HLC3 | OHLC4 | C - OHLC4".ljust(39),
                f"{0.5 * (_h + _l):.4f}, {(_h + _l + _c) / 3.:.4f}, {0.25 * (_o + _h + _l + _c):.4f}, {_c - 0.25 * (_o + _h + _l + _c):.4f}".rjust(40))
            print(
                f"Change (%)".ljust(39),
            ...
            if "bid" in ticker_info and ticker_info['bid'] is not None \
                ...
                print(
                    ...
                    f"{ticker_info['bid']} x {ticker_info['bidSize']} | {ticker_info['ask']} x {ticker_info['askSize']} | {ticker_info['ask'] - ticker_info['bid']:.4f}".rjust(40))
            print(f"Volume | Market | Avg Vol (10Day)".ljust(40))
            print(
                f"{ticker_info['volume']:,} | {_v:,} | {ticker_info['averageVolume']:,} ({ticker_info['averageDailyVolume10Day']:,})".rjust(80))
            ...
            if "fiftyTwoWeekHigh" in ticker_info and "fiftyTwoWeekLow" in ticker_info and "previousClose" in ticker_info:  # or 'regularMarketPrice'
                print(
                    ...
                    f"{ticker_info['fiftyTwoWeekLow']} - {ticker_info['fiftyTwoWeekHigh']} : {ticker_info['fiftyTwoWeekHigh'] - ticker_info['fiftyTwoWeekLow']:.4f} ({100 * (ticker_info['regularMarketPrice'] / ticker_info['fiftyTwoWeekLow'] - 1):.4f}%)".rjust(40))
            ...
            if "beta" in ticker_info and ticker_info[
                    'beta'] is not None and "beta3Year" in ticker_info and ticker_info['beta3Year'] is not None:
                print(
                    f"Beta | 3Yr".ljust(39),
            ...
            if "threeYearAverageReturn" in ticker_info and ticker_info[
                    'threeYearAverageReturn'] is not None and "fiveYearAverageReturn" in ticker_info and ticker_info['fiveYearAverageReturn'] is not None:
                print(
            ...
            if not dividends.empty or not splits.empty:
                ...
                if "dividendRate" in ticker_info and ticker_info['dividendRate'] is not None and "dividendYield" in ticker_info and ticker_info[
                        'dividendYield'] is not None and "payoutRatio" in ticker_info and ticker_info['payoutRatio'] is not None:
                    print(
                ...
                if "trailingAnnualDividendRate" in ticker_info and ticker_info[
                        'trailingAnnualDividendRate'] is not None and "trailingAnnualDividendYield" in ticker_info and ticker_info['trailingAnnualDividendYield'] is not None:
                    print(
        ...
        if kind in _all + ["institutional_holders", "ih"]:
            ...
            if ihdf is not None and "Date Reported" in ihdf.columns:
                ...
                ihdf["Shares"] = ihdf.apply(
                    lambda x: f"{x['Shares']:,}", axis=1)
        ...
        if kind in _all + ["mutualfund_holders", "mfh"]:
            ...
            if mfhdf is not None and "Holder" in mfhdf.columns:
                ...
                mfhdf["Shares"] = mfhdf.apply(
                    lambda x: f"{x['Shares']:,}", axis=1)
                mfhdf["Value"] = mfhdf.apply(
                    lambda x: f"{x['Value']:,}", axis=1)
                if kind not in _all:
                    print(f"\n{ticker_info['symbol']}")
        ...
        if kind in _all + ["earnings", "earn"]:
            earndf = yfd.earnings
            if not earndf.empty:
                earndf["Revenue"] = earndf.apply(
                    lambda x: f"{x['Revenue']:,}", axis=1)
